...
Bool tglXQueryExtension(Display *dpy, int *errorb, int *event) {
    return True;
}
...
GLXContext tglXCreateContext(Display *dpy, XVisualInfo *vis, GLXContext shareList, Bool direct) {
    TinyGLXContext *ctx;
...
void tglXDestroyContext(Display *dpy, GLXContext ctx1) {
...
int tglX_resize_viewport(GLContext *c, int *xsize_ptr, int *ysize_ptr) {
    TinyGLXContext *ctx;
    int xsize, ysize;
...
Bool tglXMakeCurrent(Display *dpy, GLXDrawable drawable, GLXContext ctx1) {
    TinyGLXContext *ctx = (TinyGLXContext *)ctx1;
...
void tglXSwapBuffers(Display *dpy, GLXDrawable drawable) {
    GLContext *gl_context;
    TinyGLXContext *ctx;
